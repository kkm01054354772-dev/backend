...
public class ChatService {
        ...
        // prompt()
        public ChatResponse exam3(String userInput, String systemMessage,
                        @RequestParam(defaultValue = "gpt-4.1-mini") String model) {

                // openai chat completion 모델에서 사용하는 모든 옵션 지정 가능
        ...
        public String adGenerate(AdCopyRequest request) {
                Prompt prompt = Prompt.builder()
                                .messages(UserMessage.builder()
                                                .text(PromptFactory.render(request.name(), request.brand(),
                                                                request.strength(), request.tone(),
                                                                request.keyword(), request.value()))
                                                .build())
                                .build();
        ...
        public String exam4(String userInput, String conversionId) {
                ...
                return chatClient.prompt().advisors(a -> a.param(ChatMemory.CONVERSATION_ID, conversionId))
                                .user(userInput)
                                .call().content();
        }
