...
public class InMemoryDocumentVectorStore {
    ...
    public void addDocument(String docId, String fileText, Map<String, Object> metadata) {
        ...
        try {
            ...
            var chuncks = textSplitter.split(document);

            // 내부적으로 임베딩 변환 수행하면서 벡터 스토어에 추가
            vectorStore.add(chuncks);

        } catch (Exception e) {
            throw new DocumentProcessingException(HttpStatus.INTERNAL_SERVER_ERROR, "임베딩 및 저장 실패: " + e.getMessage(),
                    e);
        }
    }

    public List<DocumentSearchResultDto> similaritySearch(String question, int maxResults) {

        var ragRequest = SearchRequest.builder()
                .query(question)
                .topK(maxResults)
                .similarityThreshold(0.3)
                .build();

        List<Document> results = vectorStore.similaritySearch(ragRequest);
        if (results == null) {
